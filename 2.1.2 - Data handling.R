...
log(2, x = 8) #mix of positional matching and naming
log(2, 8)# w is 2 and base is 8
...
# 1:Vectors - 1-dimensional, homogeneous ----
...
# We can't do math:
mean(test)#argument is not numeric or logical: returning NA
...
#2: Matrices 2-dimensional homogeneous ----
aa <- foo1
attributes(aa)
aa
class(aa)
...
attr(aa,"dim") <- c(3,5)##it is matrix now
aa[1,5]
attributes(aa)
class(aa)
names(aa)
...
attributes(aa) <- NULL ## it not matrix
aa
...
aa
#for more control
aa <- matrix(aa,nrow=3,byrow = TRUE)
aa
...
bb <- c(10, 20)
bb %o% aa# 2 rows 6 columns
aa %o% bb# 6 rows 2 columns

#3: Array - n-dimensional,Homogenous -----
...
#4: Lists - 1-dimensional, heterogeneous ----
typeof(plant_lm)
# how many elements:
length(plant_lm)
...
plant_lm$myComment <- "But python also :)"#create new list
plant_lm$myComment
plant_lm$hello <- "Hello hi"
plant_lm$hello
plant_lm$call
plant_lm$rank
# What is class?
...
#5: Dataframes - 2-dimensional, heterogenous ----
class(PlantGrowth)
